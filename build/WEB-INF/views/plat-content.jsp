...
<div class="grid grid-2">
    ...
    <div class="card">
        ...
        <form action="${pageContext.request.contextPath}/plat/save" method="post" onsubmit="return validateForm()" class="plat-form">
            <input type="hidden" name="id" value="${plat.id}"/>
            
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">Intitulé du plat</label>
                    <input type="text" name="intitule" value="${plat.intitule}" class="form-input" required placeholder="Ex: Poulet Basquaise">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Prix (€)</label>
                    <input type="number" name="prix" value="${plat.prix}" class="form-input" required min="0" step="0.01" placeholder="0.00">
                </div>
            </div>
            
            <div class="form-group">
                <label class="form-label">Image (URL)</label>
                <input type="url" name="image" value="${plat.image}" class="form-input" placeholder="https://example.com/image.jpg">
            </div>

            <div class="compositions-section">
                <div class="section-header">
                    <h4>Compositions</h4>
                    <button type="button" class="btn btn-outline" onclick="addCompositionRow()">
                                ...
                                <div class="form-group">
                                    <select name="composantIds" class="form-select" required>
                                        ...
                                        <c:forEach var="composant" items="${composants}">
                                            <option value="${composant.id}" ${composition.composant.id == composant.id ? 'selected' : ''}>
                                                ${composant.nom} (${composant.unite.symbol})
                                            </option>
                                        </c:forEach>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <input type="number" step="0.01" name="quantites" value="${composition.quantite}" 
                                           class="form-input" required min="0.01" placeholder="Quantité">
                                </div>
                                <div class="form-group">
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-primary">
                    ...
                    <c:choose>
                        <c:when test="${not empty plat.id}">Mettre à jour</c:when>
                        <c:otherwise>Créer le plat</c:otherwise>
                    </c:choose>
                </button>
                
                <c:if test="${not empty plat.id}">
                    <a href="${pageContext.request.contextPath}/plat" class="btn btn-outline">Annuler</a>
                </c:if>
            </div>
        </form>
    </div>
